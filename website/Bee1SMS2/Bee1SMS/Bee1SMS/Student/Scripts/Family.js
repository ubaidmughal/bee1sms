$(document).ready(function () {
    ...
    var dataTable = $('#Family_data').DataTable({
        ...
        "ajax": {
            url: "Actions/actionfamily.php",
            type: "POST",
            data: { operationFamily: operationFamily }
        },
        "columnDefs": [
			{
			    "targets": [0, 1],
    ...
    $(document).on('click', '.updateFamily', function () {
        var operationFamily = "fetch_single_record";
        var FamilyId = $(this).attr("id");
        $.ajax({
            url: "Actions/actionfamily.php",
            method: "POST",
            data: { FamilyId: FamilyId, operationFamily: operationFamily },
            dataType: "json",
            success: function (data) {
                $('#FamilyModal').modal('show');
                $('#mytable').show();
