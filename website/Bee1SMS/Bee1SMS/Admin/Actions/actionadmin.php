...
if(isset($_POST["operationAdmin"]))
{
	...
	if($_POST["operationAdmin"] == "Add")
	{
        ...
        //}
		$statement = $connection->prepare("
			INSERT INTO tbluser (UserName,UserType, Email,DateReg,Password) 
			VALUES (:UserName, :UserTypes,:Email,:DateReg,:Password)
		");
		$result = $statement->execute(
			array(
	...
	if($_POST["operationAdmin"] == "Edit")
	{
		...
		$statement = $connection->prepare(
			"UPDATE tbluser
		  SET UserName = :UserName,UserType = :UserTypes, Email = :Email , DateReg = :DateReg, Password = :Password
			WHERE AdminId = :id
			"
		);
		$result = $statement->execute(
